...
Названия групп и порядок групп проверяются **при каждой загрузке**, а не один раз в версионной ветке: соответствующий блок в `migrateData()` стоит после всех веток `if (version < N)` и выполняется независимо от версии данных. (Второй безусловный доводчик в этой функции — подстановка `settings.cardView`, если поля нет; он проще и просто дописывает значение по умолчанию.)
...
Обе функции (`sanitizeGroupOrder`, `sanitizeGroupTitle`) принимают `unknown` и никогда не бросают исключение: `migrateData()` выполняется до отрисовки доски, и исключение на повреждённых данных оставило бы пользователя вообще без доски. Сообщения о починке пользователю не показываются.
