...
2. `npx semver -r "^22.12.0 || >=24.0.0" $(node -v | tr -d v)` — печатает версию, если она
   в диапазоне, и молчит с ненулевым кодом, если нет. Той же командой проверить, что диапазон
   действительно отвергает 23.x: `npx semver -r "^22.12.0 || >=24.0.0" 23.11.0` — пустой вывод.
...
- [ ] Поправить формулировку Decision 8 в tech-spec: там записан «floor of 22.12 / Node ≥ 22.12»,
