...
<div class="tm-popup-overlay" on:click={onClose} on:keydown role="presentation">
  <div class="tm-popup" on:click|stopPropagation on:keydown|stopPropagation role="dialog">
    <h3 class="tm-popup__title">Настройки доски</h3>

    <div class="tm-popup__field">
      <label class="tm-popup__label" for="tm-board-title">Название</label>
      <input id="tm-board-title" class="tm-popup__input" type="text" bind:value={title} maxlength="200" />
    </div>

    <div class="tm-popup__field">
      <label class="tm-popup__label" for="tm-board-subtitle">Описание</label>
    ...
    <div class="tm-popup__actions">
      {#if canDelete}
        {#if !confirmDelete}
          <button class="tm-popup__btn tm-popup__btn--danger" on:click={() => confirmDelete = true}>Удалить доску</button>
        {:else}
          <button class="tm-popup__btn tm-popup__btn--danger" on:click={onDelete}>Подтвердить удаление</button>
          <button class="tm-popup__btn" on:click={() => confirmDelete = false}>Нет</button>
        {/if}
      {/if}
      <div class="tm-popup__spacer"></div>
      <button class="tm-popup__btn" on:click={onClose}>Отмена</button>
      <button class="tm-popup__btn tm-popup__btn--primary" disabled={!canSave} on:click={handleSave}>Сохранить</button>
    </div>
  </div>
</div>
