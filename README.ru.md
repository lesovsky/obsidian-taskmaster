...
**Пост-задачи.** В блоке формы «После выполнения» записываются короткие пункты о том, что сделать после задачи (до 20 пунктов по 200 символов). На карточке маркер `↪ N` показывает число ещё не заведённых пунктов. Когда задача попадает в «Завершённые» — кнопкой ☑ или перетаскиванием, — каждый незаведённый пункт становится новой задачей в конце бэклога доски, а уведомление сообщает, сколько задач заведено. Кнопка «→ в бэклог» заводит пункт досрочно при сохранении формы, а отмена ☑ убирает заведённые задачи.
...
- [Node.js](https://nodejs.org) `^22.12.0 || >=24.0.0` — линия 23.x исключена (не поддерживается vitest). Версия, на которой проверен проект, зафиксирована в `.nvmrc`
- npm
